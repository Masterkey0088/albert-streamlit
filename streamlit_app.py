...
# ====== 定数・保存先 ======
MEM_PATH = "albert_memory.json"   # フィードバック保存（βはJSON）
EVENTS_PATH = "albert_events.json"# 相談ログ（匿名集計）
POLICY_PATH = "albert_policy.json"# 学校ポリシー
...
if "OPENAI_API_KEY" not in st.secrets:
    st.error("⚠️ StreamlitのSecretsに OPENAI_API_KEY を設定してください。")
    st.stop()
...
def typewriter(md_container, text: str, step=40/1000):
    """ゆっくり描画（Streamlitでは最終結果を少しずつ差し替える）"""
    buf = ""
...
# ====== 画面：フォーム ======
st.write("学校の価値観（ポリシー）に沿って、**明日ためせる具体策**を1–2分で返します。")
...
with st.form("albert_form", clear_on_submit=False):
    ...
    message = st.text_area("相談内容（状況をできるだけ具体的に）", height=110, placeholder="例）小2の算数で立ち歩きが増加。席後方から前に移動して声かけるも効果薄…")
    attempts = st.text_area("これまで試したこと（100字以内）", height=70, max_chars=100, placeholder="例）席替え、注意、係活動の導入 など")

    st.markdown("**あなたが大切にしていること（最大4つまで）**")
    values = []
    for sec, opts in GROUPED.items():
        st.caption(sec)
        selected = st.multiselect("　", opts, key=f"vals_{sec}", max_selections=None, label_visibility="collapsed")
...
② 明日ためせる行動レシピ ×3
  必須: 手順（3〜5）/ 準備物 / 所要時間 / 声かけ例 / 【根拠】 / 価値観タグ
③ 保護者への伝え方（1行要約＋丁寧文）
...
"""
    ...
    with st.expander("しっくりきませんか？ フィードバック / 追記して再生成"):
        ...
        if b2.button("追記して再生成") and refine.strip():
            # 追記を反映して再呼び出し（簡易）
            st.session_state["refine_override"] = refine
            st.rerun()
...
if ro := st.session_state.get("refine_override"):
    ...
    st.info(f"追加指定：{ro}")
    st.experimental_set_query_params()  # クエリをリセット（おまじない）
